...
@pytest.mark.unit
def test_best_of_n_forwards_protected_nets_and_varies_seed(workdir, fake_jar):
    cc = _make_cmds(workdir / "test.kicad_pcb")
    _patch_exec_mode(cc)
    seen = []
        ...
        out = cc.autoroute(
            {
                ...
                "attempts": 3,
                "seed": 900,
                "freeroutingSeedFlag": "-seed",
                "extraFreeroutingArgs": ["-inc", "GND"],
            }
        )

    assert out["success"], out
    assert [cmd[cmd.index("-seed") + 1] for cmd in seen] == ["900", "901", "902"]
    assert all(cmd[cmd.index("-inc") + 1] == "GND" for cmd in seen)
