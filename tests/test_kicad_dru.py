...
from utils.kicad_dru import (  # noqa: E402
    ...
    apply_managed_rule_block,
    build_layer_constraint_rule,
    persist_managed_rule_block,
...
def test_managed_block_preserves_unrelated_bytes_and_replaces_only_its_payload():
    existing = (
        '(version 1)\n\n; hand note (keep this)\n(rule "keep_me"\n'
        "  (condition \"A.NetName == 'X(Y)'\")\n)\n"
    )
    first = apply_managed_rule_block(existing, '(rule "cfha_first"\n)')
    second = apply_managed_rule_block(first, '(rule "cfha_second"\n)')

    assert second.startswith(existing.rstrip("\n"))
    assert '(rule "keep_me"' in second
    assert "cfha_first" not in second
    assert second.count("cfha_second") == 1
    assert second.count(CFHA_RULES_BEGIN) == 1
    assert second.count(CFHA_RULES_END) == 1


def test_managed_block_rejects_malformed_half_marker():
    import pytest

    with pytest.raises(ValueError, match="one boundary marker"):
        apply_managed_rule_block(f"(version 1)\n{CFHA_RULES_BEGIN}\n", "")
...
def test_persist_managed_block_is_atomic_and_preserves_user_rules(tmp_path):
    dru = tmp_path / "proj.kicad_dru"
    original = '(version 1)\n\n(rule "keep_me"\n' "  (constraint clearance (min 0.3mm))\n)\n"
    dru.write_text(original, encoding="utf-8")

    result = persist_managed_rule_block(str(dru), '(rule "cfha_generated"\n)')

    assert result["persisted"] is True
    content = dru.read_text(encoding="utf-8")
    assert content.startswith(original.rstrip("\n"))
    assert '(rule "keep_me"' in content
    assert '(rule "cfha_generated"' in content
    assert [path.name for path in tmp_path.iterdir()] == ["proj.kicad_dru"]
