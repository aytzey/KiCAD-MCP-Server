...
from commands.autoroute_cfha import (
    AutorouteCFHACommands,
    _clearance_condition_for_nets,
    _condition_for_nets,
    compile_kicad_dru,
    compute_weighted_qor_score,
)


def _mock_native_connectivity(monkeypatch, unconnected_count=0):
    connectivity = MagicMock()
    connectivity.GetUnconnectedCount.return_value = unconnected_count
    monkeypatch.setattr(
        "commands.autoroute_cfha.pcbnew.CONNECTIVITY_DATA",
        lambda: connectivity,
    )
    return connectivity
...
def test_generate_kicad_dru_preserves_user_rules_and_updates_managed_block(tmp_path):
    board_path = tmp_path / "demo.kicad_pcb"
    rules_path = tmp_path / "demo.kicad_dru"
    user_rule = (
        '(version 1)\n\n; keep hand-authored content\n(rule "keep_me"\n'
        "  (constraint clearance (min 0.3mm))\n)\n"
    )
    rules_path.write_text(user_rule, encoding="utf-8")
    ...
    first = commands.generate_kicad_dru({"constraintsResult": base})
    base["constraints"]["compiledRules"][0]["min"] = 0.25
    second = commands.generate_kicad_dru({"constraintsResult": base})

    assert first["success"] is True
    assert second["success"] is True
    content = rules_path.read_text(encoding="utf-8")
    assert content.startswith(user_rule.rstrip("\n"))
    assert '(rule "keep_me"' in content
    assert content.count('(rule "cfha_generated"') == 1
    assert "0.2mm" not in content
    assert "0.25mm" in content


def test_rule_conditions_keep_hostile_net_names_inside_string_literals():
    hostile = "RF' || A.NetName == 'GND\n\"\\"

    condition = _condition_for_nets([hostile])
    clearance = _clearance_condition_for_nets([hostile], exclude_nets=["CLK'"])

    assert condition == (
        "A.NetName == 'RF\\u0027 || A.NetName == " "\\u0027GND\\u000A\\u0022\\u005C'"
    )
    assert "B.NetName != 'CLK\\u0027'" in clearance
    text = compile_kicad_dru(
        {
            "compiledRules": [
                {
    ...
    assert '(rule "hostile\\"rule"' in text
    assert "RF\\\\u0027" in text


def test_compile_kicad_dru_rejects_multiline_raw_conditions():
...
def test_detect_backends_prefers_external_orthoroute_with_gpu(monkeypatch):
    ...
    monkeypatch.setattr(
        "shutil.which",
        lambda name: name if name in {"nvidia-smi", "/usr/local/bin/orthoroute"} else None,
    )
...
def test_generate_constraints_clamps_power_rule_to_existing_board_width(tmp_path):
    ...
    power_rule = next(
        rule
        for rule in result["constraints"]["compiledRules"]
        if rule["name"] == "cfha_power_min_width"
    )
...
def test_generate_constraints_auto_infers_bus_groups_and_allows_user_override(tmp_path):
    ...
    assert result["success"] is True
    groups = {tuple(group["nets"]): group for group in result["constraints"]["matchedLengthGroups"]}
...
def test_generate_constraints_prefers_edge_low_pressure_signal_layer_for_reference(tmp_path):
    ...
    assert (
        planning["signalLayerCandidates"][0]["weightedEdgePressure"]
        < planning["signalLayerCandidates"][1]["weightedEdgePressure"]
    )
...
def test_generate_constraints_carries_placement_routing_corridors(tmp_path):
    ...
    assert (
        result["constraints"]["policy"]["placementCoupling"]["hasRoutingCorridorReservations"]
        is True
    )
    assert (
        result["constraints"]["policy"]["placementCoupling"]["routingCorridorReservationCount"] == 1
    )
...
def test_cfha_bulk_route_reuses_best_of_n_and_protects_excluded_nets(monkeypatch, tmp_path):
    board = MagicMock()
    board_path = tmp_path / "demo.kicad_pcb"
    ...
    freerouting.autoroute.return_value = {
        "success": True,
        "best_attempt": 2,
        "best_score": 42000,
        "net_protection": {
    ...
    commands = AutorouteCFHACommands(board=board, freerouting_commands=freerouting)
    monkeypatch.setattr(commands, "_ensure_board", lambda params: (board, board_path, None))
    monkeypatch.setattr(
        commands,
        "_probe_freerouting_capabilities",
        lambda jar, mode: {"inc": True, "seed": True},
    )

    result = commands.run_freerouting(
        {
            "attempts": 3,
            "seed": 70,
            "constraintsResult": {
                "success": True,
                "constraints": {
                    "criticalClasses": ["RF", "HS_DIFF"],
                    "intentGroups": {"RF": ["RF_IN"], "HS_DIFF": ["USB_D+", "USB_D-"]},
    ...
    assert result["success"] is True
    assert result["exclusionsApplied"] is True
    forwarded = freerouting.autoroute.call_args.args[0]
    assert forwarded["attempts"] == 3
    assert forwarded["targetNets"] == ["RF_IN", "USB_D+", "USB_D-"]
    assert forwarded["extraFreeroutingArgs"] == []
    assert forwarded["protectedNets"] == ["GND"]
    assert forwarded["deferSave"] is True
    assert forwarded["freeroutingSeedFlag"] == "-seed"
    assert freerouting.board is board
...
@pytest.mark.parametrize(
    ("failed_stage", "expected_failure"),
    [("bulk", "bulk"), ("post", "postTune")],
)
def test_hybrid_pipeline_propagates_bulk_and_post_failures(
    monkeypatch, tmp_path, failed_stage, expected_failure
):
    board_path = tmp_path / "stage-failure.kicad_pcb"
    board = MagicMock()
    board.GetFileName.return_value = str(board_path)
    commands = AutorouteCFHACommands(board=board)
    monkeypatch.setattr(commands, "_ensure_board", lambda params: (board, board_path, None))
    monkeypatch.setattr(
        ...
        lambda params: {
            "success": True,
            "summary": {"copperLayers": ["F.Cu", "B.Cu"]},
            "backends": {"freerouting_ready": True},
        },
    )
    monkeypatch.setattr(
        commands,
        ...
        lambda params: {
            "success": True,
            "intents": [{"net_name": "N1", "intent": "SIGNAL"}],
        },
    )
    monkeypatch.setattr(
        commands,
        ...
        lambda params: {
            "success": True,
            "constraintsPath": str(board_path.with_suffix(".routing_constraints.json")),
            "constraints": {"referencePlanning": {}},
        },
    )
    monkeypatch.setattr(
        commands,
        ...
        lambda params: {
            "success": True,
            "rulesPath": str(board_path.with_suffix(".kicad_dru")),
        },
    )
    monkeypatch.setattr(
        commands,
        "_prepare_pre_route_reference",
        lambda *args, **kwargs: {"success": True, "criticalLayer": "F.Cu"},
    )
    monkeypatch.setattr(commands, "route_critical_nets", lambda params: {"success": True})
    monkeypatch.setattr(
        commands,
        "_completion_snapshot",
        lambda *args, **kwargs: {"completionRate": 0.0},
    )
    monkeypatch.setattr(
        commands,
        "run_freerouting",
        lambda params: {
            "success": failed_stage != "bulk",
            "message": "simulated bulk result",
        },
    )
    monkeypatch.setattr(
        commands,
        "post_tune_routes",
        lambda params: {
            "success": failed_stage != "post",
            "message": "simulated post result",
        },
    )
    monkeypatch.setattr(
        commands,
    ...
    result = commands.autoroute_cfha({"boardPath": str(board_path), "strategy": "hybrid"})

    assert result["success"] is False
    assert result["stageFailures"] == [expected_failure]
    assert result["stages"][expected_failure]["success"] is False
    ...
    result = commands.route_critical_nets(
        {
            "constraintsResult": {
                ...
                "constraints": {
                    ...
                    "boardSummary": {
                        "edgePressureByLayer": {"F.Cu": {"left": 4.5, "right": 0.5, "center": 0.0}}
                    },
                    "placementRoutingCorridors": [
                        {
                            "id": "fast-left",
...
def test_route_critical_nets_locks_diff_pair_to_shared_layer(monkeypatch, tmp_path):
    ...
    def _route_diff_pair(
        net_pos, net_neg, *, inventory, constraints, width_mm, layer, board, footprints
    ):
    ...
    monkeypatch.setattr(commands, "_estimate_breakout_pressure", lambda net, pads, board, fps: 0.0)
    monkeypatch.setattr(
        commands, "_estimate_reference_alignment_pressure", lambda net, pads, board, planning: 0.0
    )
...
def test_route_critical_nets_uses_budget_safe_diff_pair_transitions(monkeypatch, tmp_path):
    ...
    board.GetLayerName.side_effect = lambda layer_id: {0: "F.Cu", 1: "In1.Cu", 31: "B.Cu"}.get(
        layer_id, "F.Cu"
    )
...
def test_route_critical_nets_respects_diff_pair_transition_cell_budget(monkeypatch, tmp_path):
    ...
    board = MagicMock()
    board.GetFileName.return_value = str(
        tmp_path / "diff_pair_transition_cell_budget_demo.kicad_pcb"
    )
    board.GetBoardEdgesBoundingBox.return_value = bbox
    ...
    monkeypatch.setattr(
        commands, "_ensure_board", lambda params: (board, Path(board.GetFileName()), None)
    )
    monkeypatch.setattr(commands, "_collect_inventory", lambda _board: inventory)
    monkeypatch.setattr(commands, "_estimate_net_congestion", lambda pads, _board: 0.0)
    monkeypatch.setattr(commands, "_estimate_escape_complexity", lambda net, pads, fps: 0.0)
...
def test_route_critical_nets_prefers_endpoint_layer_when_via_transition_penalty_is_higher(
...
@pytest.mark.parametrize(
    "failed_step",
    ["matchedLengthTuning", "referenceZone", "zoneRefill", "healing"],
)
def test_post_tune_routes_propagates_nested_step_failures(monkeypatch, tmp_path, failed_step):
    board = MagicMock()
    board.GetFileName.return_value = str(tmp_path / "post-failure.kicad_pcb")
    commands = AutorouteCFHACommands(board=board)
    monkeypatch.setattr(
        commands,
        "_ensure_board",
    ...
    def step_result(name):
        if name == failed_step:
            return {"success": False, "message": f"simulated {name} failure"}
        return {"success": True, "message": f"{name} completed"}

    monkeypatch.setattr(
        commands,
        "_tune_matched_length_groups",
        lambda *args, **kwargs: step_result("matchedLengthTuning"),
    )
    monkeypatch.setattr(
        commands,
        "_ensure_reference_ground_zone",
        lambda *args, **kwargs: step_result("referenceZone"),
    )
    monkeypatch.setattr(
        commands,
        "_refill_zones",
        lambda *args, **kwargs: step_result("zoneRefill"),
    )
    monkeypatch.setattr(
        commands,
        "_heal_support_net_connectivity",
        lambda *args, **kwargs: step_result("healing"),
    )

    result = commands.post_tune_routes(
        {
            "refillZones": True,
            "autoHealSupportNets": True,
            "constraintsResult": {
                "constraints": {
                    "matchedLengthGroups": [
                        {"nets": ["DQ0", "DQ1"], "maxSkewMm": 0.2, "type": "bus"}
                    ]
                }
            },
        }
    ...
    assert result["success"] is False
    assert result["message"] == "One or more post-route tuning steps failed"
    assert [failure["step"] for failure in result["failedSteps"]] == [failed_step]
    assert "simulated" in result["failedSteps"][0]["message"]
...
def test_post_tune_routes_adds_matched_length_compensation(monkeypatch, tmp_path):
    ...
    board.Delete.assert_called_once_with(track)
...
def test_verify_routing_qor_reports_matched_group_skew(monkeypatch, tmp_path):
    _mock_native_connectivity(monkeypatch)
    board = MagicMock()
    board.GetFileName.return_value = str(tmp_path / "demo.kicad_pcb")
    board.GetTracks.return_value = []
    board.GetFootprints.return_value = []
...
def test_verify_routing_qor_flags_transition_cell_budget_risk(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        commands, "_board_layers", lambda _board: ["F.Cu", "B.Cu", "In1.Cu", "In2.Cu"]
    )
    monkeypatch.setattr(
        commands,
        "_collect_inventory",
...
def test_autoroute_cfha_analysis_only_skips_mutating_stages(tmp_path):
    ...
    commands.generate_kicad_dru = MagicMock(
        return_value={"success": True, "path": str(tmp_path / "demo.kicad_dru")}
    )
    ...
    assert result["artifacts"]["rulesPath"] is None
    commands.generate_kicad_dru.assert_not_called()
...
def test_autoroute_cfha_hybrid_avoids_prebulk_drc_when_freerouting_unavailable(tmp_path):
    ...
    assert (
        result["stages"]["bulk"]["message"] == "Bulk router unavailable; Freerouting is not ready"
    )
...
def test_route_critical_nets_uses_reference_planning_layer_fallback(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        commands, "_estimate_escape_complexity", lambda net_name, pads, footprints: 0.0
    )
    monkeypatch.setattr(
        commands, "_estimate_breakout_pressure", lambda net_name, pads, board, footprints: 0.0
    )
...
def test_ensure_reference_ground_zone_creates_selected_domain_even_if_other_ground_zone_exists(
    tmp_path,
):
    board = MagicMock()
...
def test_autoroute_cfha_hybrid_runs_pre_route_reference_stage(tmp_path):
    ...
    commands.route_critical_nets = MagicMock(
        return_value={"success": True, "routed": [], "skipped": []}
    )
    commands.post_tune_routes = MagicMock(return_value={"success": True})
    commands._completion_snapshot = MagicMock(
        return_value={
