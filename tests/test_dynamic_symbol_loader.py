...
def test_create_component_instance_can_preserve_explicit_coordinates():
    ...
    assert ok is True
    content = schematic.read_text(encoding="utf-8")
    match = re.search(r'\(symbol \(lib_id "Device:R"\) \(at ([\d.]+) ([\d.]+) 0\)', content)
    assert match is not None
    assert (float(match.group(1)), float(match.group(2))) == (10.0, 10.0)
...
def test_power_symbols_are_marked_as_non_board_items():
    ...
    assert "(in_bom no)" in content
    assert "(on_board no)" in content
    assert "(in_pos_files no)" in content
...
def test_add_component_places_fields_outside_resistor_body():
    ...
    assert ok is True
    content = schematic.read_text(encoding="utf-8")
    assert re.search(r'\(property "Reference" "R1"\s+\(at 13\.716 8\.89 0\)', content)
    assert re.search(r'\(property "Value" "10k"\s+\(at 6\.604 11\.43 0\)', content)
