...
def test_should_auto_place_missing_footprints_defaults_to_blank_boards():
    ...
    assert (
        interface._should_auto_place_missing_footprints({"autoPlaceMissingFootprints": True}, 3)
        is True
    )
    assert (
        interface._should_auto_place_missing_footprints({"autoPlaceMissingFootprints": False}, 0)
        is False
    )
...
def test_build_auto_place_plan_profiles_connectors_to_top_bottom_and_sides():
    ...
    assert any(
        cluster["anchor"] == "J2"
        and cluster["edge"] == "top"
        and cluster["signalProfile"] == "analog"
        for cluster in plan["clusters"]
    )
    assert any(
        cluster["anchor"] == "J3"
        and cluster["edge"] == "bottom"
        and cluster["signalProfile"] == "power_switching"
        for cluster in plan["clusters"]
    )
...
def test_build_auto_place_plan_prioritizes_reference_sensitive_connectors_for_center_side_slots():
    ...
    assert abs(by_ref["J1"]["position"]["y"] - center_y) <= abs(
        by_ref["J2"]["position"]["y"] - center_y
    )
    assert abs(by_ref["J1"]["position"]["y"] - center_y) <= abs(
        by_ref["J3"]["position"]["y"] - center_y
    )
...
def test_build_auto_place_plan_partitions_quiet_and_noisy_reference_domains_on_side_edges():
    ...
    netlist = {
        "nets": [
            {
                "name": "USB1_DP",
                "connections": [{"component": "J1", "pin": "1"}, {"component": "U1", "pin": "1"}],
            },
            {
                "name": "AGND",
                "connections": [{"component": "J1", "pin": "A1"}, {"component": "U1", "pin": "2"}],
            },
            {
                "name": "USB2_DP",
                "connections": [{"component": "J2", "pin": "1"}, {"component": "U2", "pin": "1"}],
            },
            {
                "name": "GND",
                "connections": [{"component": "J2", "pin": "A1"}, {"component": "U2", "pin": "2"}],
            },
            {
                "name": "USB3_DP",
                "connections": [{"component": "J3", "pin": "1"}, {"component": "U3", "pin": "1"}],
            },
            {
                "name": "PGND",
                "connections": [{"component": "J3", "pin": "A1"}, {"component": "U3", "pin": "2"}],
            },
            {
                "name": "USB4_DP",
                "connections": [{"component": "J4", "pin": "1"}, {"component": "U4", "pin": "1"}],
            },
            {
                "name": "USB5_DP",
                "connections": [{"component": "J5", "pin": "1"}, {"component": "U5", "pin": "1"}],
            },
        ]
    }
    ...
    assert abs(by_ref["J2"]["position"]["y"] - center_y) < abs(
        by_ref["J3"]["position"]["y"] - center_y
    )
