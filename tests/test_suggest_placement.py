...
def test_multistart_is_never_worse_than_baseline_rank():
    baseline = _two_converters_board().suggest_placement({"iterations": 120, "starts": 1})
    multistart = _two_converters_board().suggest_placement({"iterations": 120, "starts": 4})

    baseline_rank = (
        baseline["score"]["overlaps_after"],
        baseline["score"]["hpwl_after_mm"],
    )
    multistart_rank = (
        multistart["score"]["overlaps_after"],
        multistart["score"]["hpwl_after_mm"],
    )
    assert multistart_rank <= baseline_rank
    assert len(multistart["summary"]["candidates"]) == 4
    assert 1 <= multistart["summary"]["selected_start"] <= 4


def test_rejects_unbounded_work_controls_and_non_quarter_turns():
    host = _two_converters_board()

    for params, field in (
        ({"iterations": 1_001}, "iterations"),
        ({"rotation_passes": 9}, "rotation_passes"),
        ({"spread_iters": 501}, "spread_iters"),
        ({"legalize_iters": 501}, "legalize_iters"),
        ({"rotation_steps": [0, 45, 90]}, "rotation_steps"),
    ):
        result = host.suggest_placement(params)
