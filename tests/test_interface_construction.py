...
def test_every_typescript_bridge_command_has_a_python_route(iface) -> None:
    """Every literal callKicadScript command exposed by TypeScript must exist."""
    tools_dir = Path(__file__).parent.parent / "src" / "tools"
    bridged = set()
    for path in tools_dir.glob("*.ts"):
        text = path.read_text(encoding="utf-8")
        bridged.update(re.findall(r'callKicadScript\(\s*["\']([a-z0-9_]+)["\']', text))

    missing = sorted(bridged - set(iface.command_routes))
    assert not missing, f"TypeScript tools call missing Python routes: {missing}"
