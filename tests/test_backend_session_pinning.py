...
@pytest.mark.unit
class TestCFHABackendOwnership:
    @staticmethod
    def _install_cfha(iface, method_name="route_critical_nets"):
        cfha = MagicMock()
        getattr(cfha, method_name).return_value = {"success": True}
        cfha.board = iface.board
        iface.autoroute_cfha_commands = cfha
        return cfha

    def test_swig_session_never_receives_live_ipc_adapter(self, tmp_path):
        board_path = tmp_path / "board.kicad_pcb"
        board_path.write_text("(kicad_pcb)", encoding="utf-8")
        iface = _make_iface({}, _FakeIPCBackend(str(board_path)), board=_FakeBoard(board_path))
        iface.session_backend = "swig"
        iface.session_board_path = iface._normalize_board_path(board_path)
        cfha = self._install_cfha(iface, "analyze_board_routing_context")

        result = iface._handle_cfha_command({}, "analyze_board_routing_context")

        assert result["success"] is True
        cfha.set_ipc_board_api.assert_called_once_with(None)

    def test_ipc_owned_session_refuses_cfha_before_any_stage(self, tmp_path):
        board_path = tmp_path / "board.kicad_pcb"
        iface = _make_iface({}, _FakeIPCBackend(str(board_path)), board=_FakeBoard(board_path))
        iface.session_backend = "ipc"
        iface.session_board_path = iface._normalize_board_path(board_path)
        cfha = self._install_cfha(iface)

        result = iface._handle_cfha_command({}, "route_critical_nets")

        assert result["success"] is False
        assert "SWIG/file-owned" in result["message"]
        cfha.route_critical_nets.assert_not_called()

    def test_pinned_session_rejects_a_different_board_path_without_loading(self, tmp_path):
        active = tmp_path / "active.kicad_pcb"
        requested = tmp_path / "other.kicad_pcb"
        iface = _make_iface({}, None, use_ipc=False, board=_FakeBoard(active))
        iface.session_backend = "swig"
        iface.session_board_path = iface._normalize_board_path(active)
        iface._safe_load_board = MagicMock()
        cfha = self._install_cfha(iface)

        result = iface._handle_cfha_command({"boardPath": str(requested)}, "route_critical_nets")
    ...
    def test_unowned_explicit_board_becomes_swig_session(self, tmp_path):
        board_path = tmp_path / "board.kicad_pcb"
        board_path.write_text("(kicad_pcb)", encoding="utf-8")
        loaded = _FakeBoard(board_path)
        iface = _make_iface({}, _FakeIPCBackend(str(board_path)), board=None)
        iface._safe_load_board = MagicMock(return_value=loaded)
        record_signature = MagicMock()
        iface._record_board_signature = record_signature
        cfha = self._install_cfha(iface, "analyze_board_routing_context")
        cfha.board = loaded

        result = iface._handle_cfha_command(
            {"boardPath": str(board_path)}, "analyze_board_routing_context"
        )
    ...
    def test_external_board_change_refuses_mutating_cfha_before_stage(self, tmp_path):
        board_path = tmp_path / "board.kicad_pcb"
        board_path.write_text("original", encoding="utf-8")
        iface = _make_iface({}, None, use_ipc=False, board=_FakeBoard(board_path))
        iface.session_backend = "swig"
        iface.session_board_path = iface._normalize_board_path(board_path)
        iface._board_disk_signature = iface._disk_signature(str(board_path))
        board_path.write_text("external edit", encoding="utf-8")
        cfha = self._install_cfha(iface)
        ...
        assert board_path.read_text(encoding="utf-8") == "external edit"
        cfha.route_critical_nets.assert_not_called()
