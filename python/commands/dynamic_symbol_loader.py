...
def _snap_schematic_coordinate(value: float, grid: float = 1.27) -> float:
    """Snap a coordinate to a positive schematic grid without float dust."""
    grid = float(grid)
    if grid <= 0:
        raise ValueError("schematic grid must be greater than zero")
    return round(round(float(value) / grid) * grid, 4)


def _symbol_board_flags(library_name: str, reference: str) -> Tuple[str, str, str]:
    """Return in-BOM, on-board, and position-file flags for an instance."""
    if library_name.casefold() == "power" or reference.startswith("#"):
        return "no", "no", "no"
    return "yes", "yes", "yes"


def _body_aware_field_positions(
    schematic_path: Path,
    full_lib_id: str,
    x: float,
    y: float,
    ...
    """Place visible fields outside the rendered body when geometry is known."""
    try:
        ...
        lib_data = _extract_lib_symbols(_load_sexp(schematic_path)).get(full_lib_id)
        ...
        if (max_y - min_y) > (max_x - min_x) * 1.4:
            return {
                "Reference": (round(max_x + margin, 4), round(center_y - 1.27, 4), 0.0),
                "Value": (round(min_x - margin, 4), round(center_y + 1.27, 4), 0.0),
            }
        return {
            "Reference": (round(min_x, 4), round(min_y - margin, 4), 0.0),
            "Value": (round(min_x, 4), round(max_y + margin, 4), 0.0),
        }
    except Exception:
        logger.debug("Could not derive body-aware field positions", exc_info=True)
        return None
...
class DynamicSymbolLoader:
    ...
    def create_component_instance(
        ...
        if snap_to_grid:
            snapped_x = _snap_schematic_coordinate(x, grid)
            snapped_y = _snap_schematic_coordinate(y, grid)
            if (snapped_x, snapped_y) != (x, y):
                logger.info(
                    "Snapped %s origin (%s, %s) -> (%s, %s) onto %smm grid",
                    reference,
                    x,
                    y,
                    snapped_x,
        ...
        body_fields = _body_aware_field_positions(
            schematic_path,
            full_lib_id,
            x,
            y,
        ...
        if body_fields:
            ref_x, ref_y, ref_a = body_fields["Reference"]
            val_x, val_y, val_a = body_fields["Value"]
        ...
        in_bom, on_board, in_pos_files = _symbol_board_flags(library_name, reference)
        instance_block = (
            f'  (symbol (lib_id "{escape_sexpr_string(full_lib_id)}")'
            f" (at {_fmt(x)} {_fmt(y)} {_fmt(angle)})"
            f"{mirror_str} (unit {unit})\n"
            f"    (body_style 1) (exclude_from_sim no) (in_bom {in_bom})"
            f" (on_board {on_board}) (in_pos_files {in_pos_files}) (dnp no)\n"
            f'    (uuid "{new_uuid}")\n'
            f"{body}\n"
            "  )"
        )
