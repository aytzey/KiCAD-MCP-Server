...
MAX_PLACEMENT_ITERATIONS = 1_000
MAX_ROTATION_PASSES = 8
MAX_AUXILIARY_ITERATIONS = 500
MAX_ROTATION_STEPS = 16
...
class PlacementOptimizerCommands:
    ...
    def suggest_placement(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            rotation_steps = [
                float(step) for step in params.get("rotation_steps", [0, 90, 180, 270])
            ]
            rotation_passes = int(params.get("rotation_passes", 2))
            starts = int(params.get("starts", 3))
            spread_iters = int(params.get("spread_iters", 80))
            legalize_iters = int(params.get("legalize_iters", 200))
            apply = bool(params.get("apply", False))

            if not 0 <= iterations <= MAX_PLACEMENT_ITERATIONS:
                return {
                    "success": False,
                    "message": (f"iterations must be between 0 and {MAX_PLACEMENT_ITERATIONS}"),
                }
            if not 0 <= rotation_passes <= MAX_ROTATION_PASSES:
                return {
                    "success": False,
                    "message": (f"rotation_passes must be between 0 and {MAX_ROTATION_PASSES}"),
                }
            if not 0 <= spread_iters <= MAX_AUXILIARY_ITERATIONS:
                return {
                    "success": False,
                    "message": (f"spread_iters must be between 0 and {MAX_AUXILIARY_ITERATIONS}"),
                }
            if not 0 <= legalize_iters <= MAX_AUXILIARY_ITERATIONS:
                return {
                    "success": False,
                    "message": (f"legalize_iters must be between 0 and {MAX_AUXILIARY_ITERATIONS}"),
                }
            if (
                not rotation_steps
                or len(rotation_steps) > MAX_ROTATION_STEPS
                or any(
                    not math.isfinite(step) or abs(step / 90.0 - round(step / 90.0)) > 1e-9
                ...
                return {
                    "success": False,
                    "message": (
                        "rotation_steps must contain 1 to "
                        f"{MAX_ROTATION_STEPS} finite multiples of 90 degrees"
                    ),
                }
            rotation_steps = list(dict.fromkeys(step % 360.0 for step in rotation_steps))
            if grid_mm <= 0 or margin_mm < 0:
                return {"success": False, "message": "grid_mm must be > 0 and margin_mm >= 0"}
            if not 1 <= starts <= 12:
                return {"success": False, "message": "starts must be between 1 and 12"}

            locked = set(params.get("locked") or [])
            params = {
                **params,
                "spread_iters": spread_iters,
                "legalize_iters": legalize_iters,
            }
            ...
            initial_nodes = self._opt_clone_nodes(nodes)

            edges = self._opt_build_edges(
                nodes, net_names, power_nets, power_weight, decoupling_boost
            )

            hpwl_before = self._opt_hpwl(initial_nodes)
            overlaps_before = self._opt_count_overlaps(initial_nodes, margin_mm)

            # Baseline candidate preserves the current placement as its seed.
            self._opt_run_candidate(
                nodes,
                edges,
                region,
                params=params,
            ...
            # Candidate pipeline also performs spreading, alignment, grid snapping,
            # and final courtyard legalization.
            hpwl_after = self._opt_hpwl(nodes)
            overlaps_after = self._opt_count_overlaps(nodes, margin_mm)
            boundary_after = self._opt_count_boundary_violations(nodes, region)
            candidate_results = [
                {
                    "start": 1,
                    "boundary_violations": boundary_after,
                    "overlaps": overlaps_after,
                    "hpwl_mm": round(hpwl_after, 2),
                }
            ]
            best_nodes = nodes
            best_rank = (boundary_after, overlaps_after, hpwl_after)
            selected_start = 1

            for start_index in range(1, starts):
                candidate = self._opt_clone_nodes(initial_nodes)
                self._opt_seed_candidate(candidate, region, start_index)
                self._opt_run_candidate(
                    candidate,
                    edges,
                    region,
                ...
                candidate_hpwl = self._opt_hpwl(candidate)
                candidate_overlaps = self._opt_count_overlaps(candidate, margin_mm)
                candidate_boundary = self._opt_count_boundary_violations(candidate, region)
                candidate_results.append(
                    {
                        "start": start_index + 1,
                        "boundary_violations": candidate_boundary,
                        "overlaps": candidate_overlaps,
                        "hpwl_mm": round(candidate_hpwl, 2),
                    }
                )
                rank = (candidate_boundary, candidate_overlaps, candidate_hpwl)
                if rank < best_rank:
    ...
    @staticmethod
    def _opt_clone_nodes(nodes: Dict[str, Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
        """Copy mutable solver state while retaining read-only footprint handles."""
        return {ref: {**node, "pads": list(node["pads"])} for ref, node in nodes.items()}

    def _opt_seed_candidate(self, nodes, region, start_index: int) -> None:
        """Apply a deterministic low-discrepancy perturbation for a new start."""
        movable = [(ref, node) for ref, node in sorted(nodes.items()) if not node["locked"]]
        if not movable:
            return

        if region is not None:
            x1, y1, x2, y2 = region
            scale = max(0.5, min(5.0, min(x2 - x1, y2 - y1) * 0.04))
        else:
            xs = [node["x"] for _, node in movable]
            ys = [node["y"] for _, node in movable]
            scale = max(0.5, min(5.0, max(max(xs) - min(xs), max(ys) - min(ys), 10.0) * 0.04))

        golden_angle = math.pi * (3.0 - math.sqrt(5.0))
        phase = start_index * golden_angle
        for ordinal, (_, node) in enumerate(movable):
            radius = scale * (0.5 + ((ordinal * 7 + start_index * 3) % 11) / 10.0)
            angle = phase + ordinal * golden_angle
            node["x"] += radius * math.cos(angle)
            node["y"] += radius * math.sin(angle)
            if region is not None:
                x1, y1, x2, y2 = region
                node["x"] = min(
                    max(node["x"], x1 + node["hw"] - node["cox"]),
                    x2 - node["hw"] - node["cox"],
                )
                node["y"] = min(
                    max(node["y"], y1 + node["hh"] - node["coy"]),
                    y2 - node["hh"] - node["coy"],
                )

    def _opt_run_candidate(
        self,
        nodes,
        edges,
        region,
        *,
        ...
        """Run the common solve, orientation, spreading, and legalize stages."""
        ...
        if do_rotate:
            for _ in range(rotation_passes):
        ...
        if params.get("align", True):
            self._opt_align(nodes, float(params.get("align_tol_mm", 1.5)))
        for node in nodes.values():
            if not node["locked"]:
                node["x"] = round(node["x"] / grid_mm) * grid_mm
                node["y"] = round(node["y"] / grid_mm) * grid_mm
        self._opt_legalize(
            nodes,
            region,
            margin_mm,
            int(params.get("legalize_iters", 200)),
        )
    ...
    @staticmethod
    def _opt_count_boundary_violations(nodes, region) -> int:
        if region is None:
            return 0
        x1, y1, x2, y2 = region
        count = 0
