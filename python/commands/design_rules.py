...
class DesignRuleCommands:
    ...
    @staticmethod
    def _parse_drc_report_text(
        report_text: str,
    ) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
        """Parse KiCad's text DRC report into the JSON-report result shape.

        KiCad builds that do not expose ``kicad-cli pcb drc --format json``
        can still emit a report through pcbnew.  Keeping one normalized result
        format lets autorouting and manufacturing preflight use either backend.
        """
        ...
        location_re = re.compile(r"@\((?P<x>-?\d+(?:\.\d+)?)\s*mm,\s*(?P<y>-?\d+(?:\.\d+)?)\s*mm\)")
        ...
        def flush() -> None:
            ...
            violation_type = str(current.get("type", "unknown"))
    ...
    def _write_violations_file(
        ...
        *,
        ...
        """Persist a normalized DRC result beside the board."""
        import json

        board_path = Path(board_file)
        violations_file = board_path.with_name(f"{board_path.stem}_drc_violations.json")
    ...
    def _run_drc_pcbnew_fallback(
        self,
        board_file: str,
        report_path: Optional[str],
    ) -> Dict[str, Any]:
        """Run DRC through pcbnew when ``kicad-cli pcb drc`` is unavailable."""
        import tempfile

        temporary_report: Optional[str] = None
        if report_path:
            report_output = os.path.abspath(os.path.expanduser(report_path))
        else:
            handle = tempfile.NamedTemporaryFile(mode="w", suffix=".rpt", delete=False)
            report_output = handle.name
            handle.close()
            temporary_report = report_output

        try:
            writer = getattr(pcbnew, "WriteDRCReport", None)
            units = getattr(pcbnew, "EDA_UNITS_MILLIMETRES", None)
            if writer is None or units is None:
                return {
                    "success": False,
                    "message": "DRC is unavailable in this KiCad build",
                    "errorDetails": (
                        "Neither 'kicad-cli pcb drc' nor pcbnew.WriteDRCReport is available"
                    ),
                }
            if not writer(self.board, report_output, units, True) or not os.path.exists(
                report_output
            ):
                return {
                    "success": False,
            ...
            violations_file = self._write_violations_file(board_file, violations, summary)
            return {
                "success": True,
                "message": f"Found {len(violations)} DRC violations",
                "summary": summary,
                "violationsFile": violations_file,
                "reportPath": report_output if report_path else None,
                "backend": "pcbnew-report",
            }
        finally:
            if temporary_report and os.path.exists(temporary_report):
        ...
        try:
            ...
            try:
                ...
                if result.returncode != 0:
                    logger.warning(
                        "kicad-cli DRC failed (%s); trying pcbnew report fallback: %s",
                        result.returncode,
                        result.stderr,
                    )
                    fallback = self._run_drc_pcbnew_fallback(board_file, report_path)
                    if fallback.get("success"):
                        fallback["cliError"] = result.stderr
                        return fallback
                    return {
                        "success": False,
                        "message": "DRC command failed",
                        "errorDetails": result.stderr,
                        "fallbackError": fallback.get("errorDetails") or fallback.get("message"),
                    }
