...
class KiCADInterface(SchematicHandlersMixin):
    ...
    def __init__(self) -> None:
        ...
        self.manufacturing_commands = ManufacturingCommands(
            self.board,
            run_drc=self.design_rule_commands.run_drc,
            check_placement=self.component_commands.check_courtyard_overlaps,
            export_gerbers=self._handle_export_gerbers,
            export_drill=self._handle_export_drill,
            export_position=self._handle_export_pos,
            export_bom=self.export_commands.export_bom,
            export_schematic_bom=self._handle_export_sch_bom,
        )
        ...
        self.command_routes = {
            ...
            # Constraint-first hybrid routing.  These stages share one
            # canonical constraint model so analysis, critical-net routing,
            # bulk routing and final QoR cannot silently disagree.
            ...
            "analyze_manufacturing_readiness": self._handle_analyze_manufacturing_readiness,
            "prepare_manufacturing_package": self._handle_prepare_manufacturing_package,
            ...
            "polish_schematic_readability": self._handle_polish_schematic_readability,
    ...
    def _handle_cfha_command(self, params: Dict[str, Any], method_name: str) -> Dict[str, Any]:
        """Run one CFHA stage under a single SWIG/file board owner.

        CFHA still contains SWIG-only planning and editing stages.  Mixing its
        board snapshot with a live IPC board can write to the wrong GUI document
        or overwrite IPC edits, so an IPC-owned session must be explicitly
        reopened on the file backend before using this pipeline.
        """
        if getattr(self, "session_backend", None) == "ipc":
            return {
                "success": False,
                "message": "CFHA autorouting requires a SWIG/file-owned board session",
                "errorDetails": (
                    "The active project is owned by KiCad IPC. Save/close the live PCB, "
                    "then reopen the project through MCP so routing uses one board backend."
                ),
                "sessionBackend": "ipc",
            }

        board_path = params.get("boardPath")
        requested_path = self._normalize_board_path(board_path)
        pinned_path = self._normalize_board_path(getattr(self, "session_board_path", None))
        if requested_path and pinned_path and requested_path != pinned_path:
            return {
                "success": False,
                "message": "CFHA boardPath does not match the active project",
                "errorDetails": (
                    f"Active board: {self.session_board_path}; requested board: {board_path}. "
                    "Close or reopen the project before changing routing targets."
                ),
                "sessionBackend": getattr(self, "session_backend", None),
            }

        if board_path:
            target = str(Path(board_path).expanduser().resolve())
            current = ""
            if self.board and self.board.GetFileName():
                current = str(Path(self.board.GetFileName()).expanduser().resolve())
            if target != current:
                loaded = self._safe_load_board(target)
                if loaded is None:
                    return {
                        "success": False,
        ...
        current_path = self._current_board_path()
        if pinned_path and current_path and self._normalize_board_path(current_path) != pinned_path:
            return {
                "success": False,
                "message": "CFHA board state does not match the active project",
                "errorDetails": (
                    f"Active board: {self.session_board_path}; loaded board: {current_path}. "
                    "Reopen the project before routing."
                ),
                "sessionBackend": getattr(self, "session_backend", None),
            }
        if method_name in self._CFHA_BOARD_MUTATING_METHODS:
            dirty_state = self._dirty_state(current_path)
            if dirty_state.get("diskChangedExternally"):
                return {
                    "success": False,
                    "message": "CFHA routing refused because the PCB changed on disk",
                    "errorDetails": (
                        "Reload the project to incorporate the external edit before routing. "
                        "No CFHA stage was started."
                    ),
                    "boardPath": current_path,
                    **dirty_state,
                }

        # An unowned board becomes a SWIG/file session before CFHA can inspect
        # or mutate it.  Never call _pin_session_backend here: that helper may
        # choose IPC, which this mixed implementation cannot safely support.
        if getattr(self, "session_backend", None) is None and current_path:
            self.session_backend = "swig"
            self.session_board_path = self._normalize_board_path(current_path)
            self._record_board_signature()

        self.autoroute_cfha_commands.set_board(self.board)
        self.autoroute_cfha_commands.set_ipc_board_api(None)
        result = getattr(self.autoroute_cfha_commands, method_name)(params)

        # _ensure_board may have loaded a board directly when the interface had
        # no active project.  Adopt it so subsequent MCP calls see the result.
        if self.autoroute_cfha_commands.board is not self.board:
            self.board = self.autoroute_cfha_commands.board
            self._update_command_handlers()
            self.autoroute_cfha_commands.set_ipc_board_api(None)
            adopted_path = self._current_board_path()
            if getattr(self, "session_backend", None) is None and adopted_path:
                self.session_backend = "swig"
                self.session_board_path = self._normalize_board_path(adopted_path)
                self._record_board_signature()
        return result

    def _handle_analyze_board_routing_context(self, params: Dict[str, Any]) -> Dict[str, Any]:
        return self._handle_cfha_command(params, "analyze_board_routing_context")

    def _handle_extract_routing_intents(self, params: Dict[str, Any]) -> Dict[str, Any]:
        return self._handle_cfha_command(params, "extract_routing_intents")

    def _handle_generate_routing_constraints(self, params: Dict[str, Any]) -> Dict[str, Any]:
        return self._handle_cfha_command(params, "generate_routing_constraints")

    def _handle_generate_kicad_dru(self, params: Dict[str, Any]) -> Dict[str, Any]:
        return self._handle_cfha_command(params, "generate_kicad_dru")

    def _handle_route_critical_nets(self, params: Dict[str, Any]) -> Dict[str, Any]:
        return self._handle_cfha_command(params, "route_critical_nets")

    def _handle_run_freerouting(self, params: Dict[str, Any]) -> Dict[str, Any]:
        return self._handle_cfha_command(params, "run_freerouting")

    def _handle_post_tune_routes(self, params: Dict[str, Any]) -> Dict[str, Any]:
        return self._handle_cfha_command(params, "post_tune_routes")

    def _handle_verify_routing_qor(self, params: Dict[str, Any]) -> Dict[str, Any]:
        return self._handle_cfha_command(params, "verify_routing_qor")

    def _handle_autoroute_cfha(self, params: Dict[str, Any]) -> Dict[str, Any]:
        return self._handle_cfha_command(params, "autoroute_cfha")

    def _handle_autoroute_default(self, params: Dict[str, Any]) -> Dict[str, Any]:
        return self._handle_cfha_command(params, "autoroute_default")

    def _handle_manufacturing_command(
        self, params: Dict[str, Any], method_name: str
    ) -> Dict[str, Any]:
        """Run manufacturing checks/exports against one synchronized board."""
        board_path = params.get("boardPath")
        if board_path:
            target = str(Path(board_path).expanduser().resolve())
            current = self._current_board_path() or ""
            if self._normalize_board_path(target) != self._normalize_board_path(current):
                loaded = self._safe_load_board(target)
                if loaded is None:
                    return {"success": False, "message": f"Could not load board from {board_path}"}
                self.board = loaded
                self._update_command_handlers()
                self._record_board_signature()

        if method_name == "prepare_manufacturing_package" and params.get("saveBeforeExport", True):
            save_result = self._handle_save_board({})
            if not save_result.get("success"):
                return {
                    "success": False,
                    "message": "Could not save the board before manufacturing export",
                    "stage": "save",
                    "errorDetails": save_result,
                }

        manufacturing = self.manufacturing_commands
        manufacturing.set_board(self.board)
        return getattr(manufacturing, method_name)(params)

    def _handle_analyze_manufacturing_readiness(self, params: Dict[str, Any]) -> Dict[str, Any]:
        return self._handle_manufacturing_command(params, "analyze_manufacturing_readiness")

    def _handle_prepare_manufacturing_package(self, params: Dict[str, Any]) -> Dict[str, Any]:
        return self._handle_manufacturing_command(params, "prepare_manufacturing_package")

    @staticmethod
    def _handle_polish_schematic_readability(params: Dict[str, Any]) -> Dict[str, Any]:
        path = params.get("schematicPath") or params.get("path")
        if not path:
        ...
        return polish_schematic_readability(
            ...
            internal_label_font_size=float(params.get("internalLabelFontSize", 0.2)),
    ...
    def _handle_add_component_annotation(self, params: Dict[str, Any]) -> Dict[str, Any]:
        """Add a real footprint field so the annotation survives board saves."""
        if not self.board:
            return {"success": False, "message": "No board is loaded"}
        reference = params.get("reference")
        annotation = params.get("annotation")
        if not reference or not annotation:
            return {"success": False, "message": "reference and annotation are required"}
        footprint = self.board.FindFootprintByReference(reference)
        if not footprint:
            return {"success": False, "message": f"Component '{reference}' not found"}
        try:
            field_id = footprint.GetNextFieldId()
            field = pcbnew.PCB_FIELD(footprint, field_id, "Annotation")
            field.SetText(str(annotation))
            field.SetVisible(bool(params.get("visible", True)))
            ...
            logger.error("Error adding component annotation: %s", exc)
            return {"success": False, "message": str(exc)}

    def _handle_group_components(self, params: Dict[str, Any]) -> Dict[str, Any]:
        """Create a KiCad PCB group containing all found references."""
        if not self.board:
            return {"success": False, "message": "No board is loaded"}
        references = list(dict.fromkeys(params.get("references") or []))
        if not references:
            return {"success": False, "message": "references must contain at least one item"}
        try:
            group_name = str(params.get("groupName") or "Component Group")
            ...
            if missing:
                result["warnings"] = [f"Components not found: {', '.join(missing)}"]
            return result
        except Exception as exc:
            logger.error("Error grouping components: %s", exc)
            return {"success": False, "message": str(exc)}

    def _handle_replace_component(self, params: Dict[str, Any]) -> Dict[str, Any]:
        """Replace a footprint while preserving placement and matching pad nets."""
        if not self.board:
            return {"success": False, "message": "No board is loaded"}
        reference = params.get("reference")
        requested = params.get("newFootprint") or params.get("newComponentId")
        if not reference or not requested:
            return {
                "success": False,
                "message": "reference and newFootprint (or newComponentId) are required",
            }
        ...
        try:
            found = self.footprint_library.find_footprint(str(requested))
            if not found:
                return {"success": False, "message": f"Footprint '{requested}' not found"}
            library_path, footprint_name = found
            new = pcbnew.FootprintLoad(library_path, footprint_name)
            if not new:
                return {"success": False, "message": f"Could not load footprint '{requested}'"}

            connected_pads: Dict[str, Any] = {}
            for pad in old.Pads():
                if pad.GetNetCode() > 0:
                    connected_pads[str(pad.GetNumber())] = pad.GetNet()
            new_pad_numbers = {str(pad.GetNumber()) for pad in new.Pads()}
            missing_pads = sorted(set(connected_pads) - new_pad_numbers)
            if missing_pads and not params.get("allowPadMismatch", False):
                return {
                    "success": False,
                    "message": "Replacement would disconnect mapped pads",
                    "missingPads": missing_pads,
                    "hint": "Choose a pin-compatible footprint or explicitly set allowPadMismatch=true.",
                }
            for pad in new.Pads():
                net = connected_pads.get(str(pad.GetNumber()))
                if net is not None:
                    pad.SetNet(net)
            ...
            old_flipped = bool(getattr(old, "IsFlipped", lambda: False)())
            parent_group = getattr(old, "GetParentGroup", lambda: None)()
            new.SetPosition(old_position)
            new.SetReference(reference)
            new.SetValue(params.get("newValue") or old.GetValue())
            if hasattr(new, "SetAttributes") and hasattr(old, "GetAttributes"):
                new.SetAttributes(old.GetAttributes())
            if not old_flipped:
                new.SetLayer(old_layer)

            if ":" in str(requested):
                nickname, _ = str(requested).split(":", 1)
                new.SetFPID(pcbnew.LIB_ID(nickname, footprint_name))

            self.board.Add(new)
            if old_flipped and not bool(getattr(new, "IsFlipped", lambda: False)()):
                new.Flip(old_position, False)
            new.SetOrientation(old_orientation)
            try:
                delete_board_item(self.board, old)
            except Exception:
                delete_board_item(self.board, new)
                raise
            if parent_group is not None:
                parent_group.AddItem(new)

            result = {
                "success": True,
                "message": f"Replaced {reference} with {requested}",
                "component": {
                    "reference": reference,
                    "footprint": str(requested),
                    "value": new.GetValue(),
                    "preservedNetPads": sorted(set(connected_pads) & new_pad_numbers),
                },
            }
            if missing_pads:
                result["warnings"] = [f"Disconnected missing pads: {', '.join(missing_pads)}"]
            return result
        except Exception as exc:
            logger.error("Error replacing component: %s", exc)
            return {"success": False, "message": str(exc)}

    def _handle_add_zone(self, params: Dict[str, Any]) -> Dict[str, Any]:
        """Normalize the legacy zone tool's top-level unit and delegate."""
        unit = str(params.get("unit", "mm"))
        factor = {"mm": 1.0, "mil": 0.0254, "inch": 25.4}.get(unit)
        if factor is None:
            return {"success": False, "message": f"Unsupported unit: {unit}"}
        normalized = {**params}
        normalized["points"] = [
            {"x": float(point["x"]) * factor, "y": float(point["y"]) * factor, "unit": "mm"}
            for point in params.get("points", [])
        ]
        for key in ("clearance", "minWidth"):
            if params.get(key) is not None:
                normalized[key] = float(params[key]) * factor
        return self.routing_commands.add_copper_pour(normalized)
    ...
    def _update_command_handlers(self) -> None:
        ...
        # Some lifecycle tests and legacy embedders construct a lightweight
        # interface via ``__new__`` and install only the handlers they use.
        # Keep board-state cleanup safe for those partial instances.
        cfha_commands = getattr(self, "autoroute_cfha_commands", None)
        if cfha_commands is not None:
            cfha_commands.set_board(self.board)
            # CFHA is file/SWIG-owned until every stage has a fully IPC-native,
            # transactional implementation.  Reinjecting a live IPC adapter
            # here would bypass the session ownership guard (#223).
            cfha_commands.set_ipc_board_api(None)
        manufacturing_commands = getattr(self, "manufacturing_commands", None)
        if manufacturing_commands is not None:
            manufacturing_commands.set_board(self.board)
    ...
    def _handle_export_position_file(self, params: Dict[str, Any]) -> Dict[str, Any]:
        """Compatibility wrapper for the original placement-file tool."""
        units = str(params.get("units", "mm")).lower()
        if units == "mil":
            return {
                "success": False,
                "message": "KiCad CLI position export does not support mil units; use mm or inch",
            }
        side_map = {"top": "front", "bottom": "back", "both": "both"}
        normalized = {
            **params,
            "format": str(params.get("format", "CSV")).lower(),
            "units": "in" if units == "inch" else units,
            "side": side_map.get(str(params.get("side", "both")).lower(), "both"),
        }
        return self._handle_export_pos(normalized)

    def _handle_export_vrml(self, params: Dict[str, Any]) -> Dict[str, Any]:
        """Compatibility wrapper for the original VRML export tool."""
        if params.get("includeComponents") is False:
            return {
                "success": False,
                "message": (
                    "KiCad's VRML exporter cannot reliably exclude every component; "
                    "use export_3d_cli with a mesh format and boardOnly=true"
                ),
            }
        output_path = params.get("outputPath")
        normalized = {
        ...
        if params.get("useRelativePaths") and output_path:
            normalized["modelsDir"] = str(
                Path(output_path).expanduser().resolve().parent / "models"
            )
            normalized["modelsRelative"] = True
        return self._handle_export_3d_cli(normalized)
