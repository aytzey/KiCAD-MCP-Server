...
describe("formatKicadResult", () => {
  ...
  it("optionally pretty-prints JSON without changing failure detection", () => {
    const payload = { success: false, error: "boom" };
    const response = formatKicadResult(payload, 2);

    expect(response.content[0].text).toBe(JSON.stringify(payload, null, 2));
    expect(response.isError).toBe(true);
  });
});
