...
import { registerCFHATools } from "./tools/cfha.js";
...
export function findPythonExecutable(scriptPath: string): string {
  ...
  // An explicit operator choice is authoritative. In particular, a generic
  // development .venv on Windows cannot import KiCad's version-coupled
  // pcbnew.pyd and must not shadow the bundled interpreter configured here.
...
export class KiCADMcpServer {
  ...
  /** Distinguishes the pre-spawn startup window from a stopped/crashed backend. */
  private lifecycleState: "idle" | "starting" | "ready" | "failed" | "stopped" = "idle";
  ...
  private registerAll(): void {
    ...
    registerCFHATools(this.server, this.callKicadScript.bind(this));
    registerManufacturingTools(this.server, this.callKicadScript.bind(this));
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      this.pythonProcess.on("exit", (code, signal) => {
        ...
        this.markBackendFailed(
          new Error(`KiCad Python process exited with code ${code} and signal ${signal}`),
        );
      ...
      this.markBackendFailed(error instanceof Error ? error : new Error(String(error)));
      throw error;
    }
  }
  ...
  async stop(): Promise<void> {
    ...
    this.rejectPendingRequests(new Error("KiCad MCP server stopped"));
  ...
  private async callKicadScript(command: string, params: any): Promise<any> {
    return new Promise((resolve, reject) => {
      // The MCP transport intentionally connects before Python is spawned.
      // Calls in that short startup window must enter the ready-gated queue;
      // a missing process in any other lifecycle state is a real failure.
      if (!this.pythonProcess && this.lifecycleState !== "starting") {
  ...
  private markBackendFailed(error: Error): void {
    if (this.lifecycleState === "failed" || this.lifecycleState === "stopped") return;
    const backendWasStarting = this.lifecycleState === "starting" && this.pythonProcess !== null;
    this.lifecycleState = "failed";
    if (backendWasStarting) this.rejectReady(error);
