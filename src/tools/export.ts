...
export function registerExportTools(server: McpServer, callKicadScript: CommandFunction): void {
  ...
  server.tool(
    "export_vrml",
    "Export the PCB and its component models as VRML for web viewers or simulation tools. For a board-only model, use export_3d_cli with a mesh format and boardOnly=true.",
    {
      outputPath: z.string().describe("Path to save the VRML file"),
      includeComponents: z
        .literal(true)
        .optional()
        .describe("VRML includes component models; omit or set true"),
      useRelativePaths: z
        .boolean()
        .optional()
