...
export function registerComponentTools(server: McpServer, callKicadScript: CommandFunction): void {
  ...
  server.tool(
    "replace_component",
    "Replace a PCB footprint while preserving reference, value/override, placement, side, orientation, attributes, group membership, and nets on matching pad numbers. Refuses a connected-pad mismatch by default.",
    {
      ...
      allowPadMismatch: z
        .boolean()
        .optional()
        .describe("Allow replacement even when connected old pad numbers do not exist on the new footprint"),
    },
    async ({ reference, newComponentId, newFootprint, newValue, allowPadMismatch }) => {
  ...
  server.tool(
    ...
    "Propose an optimized PCB footprint placement that shortens net length, orients parts toward their partners, and removes courtyard overlaps. Force-directed clustering pulls connected parts together (a converter's feedback divider and decoupling caps end up hugging its IC), power/high-current nets are weighted short & direct, and each part is rotated (0/90/180/270) to face neighbours so airwires stop crossing. PCB ONLY — does not touch the schematic. DRY RUN by default: returns proposals {ref:[x,y,rot]} plus a score (HPWL before/after, overlap counts) without modifying the board. Validate via check_courtyard_overlaps(positions=proposals), then re-run with apply=true before autoroute.",
    {
      ...
      iterations: z
        ...
        .max(1000)
        .optional()
        .describe("Force-directed relaxation passes (default 200, maximum 1000)."),
      starts: z
        .number()
        .int()
        .min(1)
        ...
        .optional()
        .describe(
          "Deterministic multi-start candidates (default 3). The winner is chosen by boundary violations, courtyard overlaps, then pad-level HPWL.",
        ),
      grid_mm: z
        .number()
        .optional()
      ...
      rotation_steps: z
        .array(
          z
            .number()
            .finite()
            .refine((angle) => Number.isInteger(angle / 90), {
              message: "Each candidate rotation must be a multiple of 90 degrees",
            }),
        )
        .min(1)
        .max(16)
        .optional()
        .describe(
          "Candidate quarter-turn orientations in degrees (default [0, 90, 180, 270]).",
        ),
      rotation_passes: z
        .number()
        .int()
        ...
        .max(8)
        .optional()
        .describe("Greedy pin-facing rotation sweeps per start (default 2, maximum 8)."),
      spread_iters: z
        .number()
        .int()
        .nonnegative()
        .max(500)
        .optional()
        .describe("Density-spreading passes per start (default 80, maximum 500)."),
      legalize_iters: z
        .number()
        .int()
        .nonnegative()
        .max(500)
        .optional()
        .describe("Overlap-legalization passes per start (default 200, maximum 500)."),
      power_nets: z
        .array(z.string())
        .optional()
        .describe(
