...
### Production-ready two-layer PCB flow

- Merged upstream `main` through v2.7.0 while preserving the fork's placement,
  routing, schematic-polish, and project-default improvements.
- Added deterministic multi-start placement with pin-facing rotation, density
  spreading, alignment, legalization, and lexicographic boundary/overlap/HPWL
  candidate selection.
- Added constraint-first hybrid autorouting (CFHA): intent and rule synthesis,
  protected critical nets, reference-path planning, best-of-N Freerouting,
  post-route tuning, DRC, and auditable QoR scoring.
- Added `analyze_manufacturing_readiness` and
  `prepare_manufacturing_package` for conservative two-layer fab/assembly
  gates, hand-vs-SMT checks, staged Gerber/drill/BOM/position export, SHA-256
  manifests, and rollback-safe ZIP publication.
- Closed TypeScript/Python bridge gaps for five previously registered but
  unroutable tools and added persistent route-completeness tests.
